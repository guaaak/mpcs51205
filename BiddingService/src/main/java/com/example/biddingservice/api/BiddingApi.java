...
public interface BiddingApi {
    ...
    Boolean makeNewOffer(@RequestBody RequestNewOfferBody requestNewOfferBody);

    @RequestMapping(value = "/newBid", method = RequestMethod.POST)
    Boolean initBid(@RequestBody RequestInitBidBody requestInitBidBody);

    // admin forcing a bid on an item to close
    @RequestMapping(value = "/closeBid/{itemID}", method = RequestMethod.POST)
    Boolean closeBidByAdmin(@PathVariable String itemID);

    @RequestMapping(value = "/closeEmptyBid/{itemID}", method = RequestMethod.GET)
    Boolean closeEmptyBid(@PathVariable String itemID);
    ...
    Boolean closeBidFromBuyNow(@PathVariable String itemId);
